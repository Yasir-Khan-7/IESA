...
if st.session_state.df is not None:
    ...
    if st.button("Run WisRule Analsysis"):
        st.session_state.run_analyis = True

if st.session_state.run_analysis:
    with st.spinner('Running WisRule Analysis...'):
        wisrule = WisRuleWithNegative(st.session_state.transactions, min_support, min_confidence)
        ...
        if not rules:
            ...
            st.download_button(
                ...
                file_name='wisrule_output.csv',
                mime='text/csv'
            )

            st.subheader("Top 5 Recommendations")
            top_rules = rule_df.sort_values(by=["Confidence", "Utility"], ascending=False).head(5)
